...
namespace cg::domain
{
	...
	using life_point_type = std::int32_t;
	// 攻击力/防御力(不允许负数,但是设置为有符号类型可以简化计算)
	using attack_defense_value_type = std::int32_t;

	// 未知攻击力/防御力(?)
	constexpr attack_defense_value_type unknown_attack_defense_value = -1;
}
